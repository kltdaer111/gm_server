			...
			foreach ($msg_data['server'] as $server_id => $choosen) {
				...
				//进行操作
					...
					switch ($msg_data['oper']) {
						...
						//再更新
						case '不关服更新':
							update_last_server_action($db_con, $server_id, 1, 'type');
							...
							switch ($server_source) {
								case '内网Branches':
									...
									$res[$server_id]['copy'] = $this_obj->sync_operation("cat ~/sg_server/nohup_run/b.txt");
									//log_debug($res[$server_id]['copy']);
									break;
								case '内网Trunk':
							...
							if($res[$server_id]['copy'] != ''){
								update_last_server_action($db_con, $server_id, 3, 'type');
							}
							else{
								update_last_server_action($db_con, $server_id, 2, 'type');
							}
							break;
					}
				}
